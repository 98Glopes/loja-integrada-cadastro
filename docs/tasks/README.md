...
| 11 | [`ProcessarLote`: orquestração, relatório e comando `processar` (com dummy)](11-processar-lote-relatorio.md) | 08, 09, 10 | sonnet | concluída | [spec](../specs/tasks/11-processar-lote-relatorio.md) — laço fotos→textos→montar por produto, `Ctrl+C` gera planilha/relatório parciais, `--refazer-fotos`/`--refazer-textos` retomam mesmo a partir de `pronto`; 265 testes. Critério de aceite com R2 real/importação manual fica pendente de execução pelo usuário |
| 12 | [Cliente LLM Anthropic e repositório de prompts](12-cliente-llm-prompts.md) | 02 | opus | concluída | [spec](../specs/tasks/12-cliente-llm-prompts.md) — `beta.messages.parse` com dataclass direto, `fallbacks="default"` (ADR-010), custo por `precos_llm.yaml`; integração real com cache hit; 34 testes novos |
| 13 | [Recursos de conteúdo: loja, marcas, copy, SEO, QA](13-recursos-conteudo.md) | 12 | opus | concluída | [spec](../specs/tasks/13-recursos-conteudo.md) — `loja/copy/seo/qa.md` + 9 perfis de marca + `_generico.md` sem URL/foto/sufixo; `perfil_marca()` → `PerfilMarca` com flag `generico`, `marcas_com_perfil()` ligado ao `validar`, `palavras_proibidas()` parseado de `seo.md`; 341 testes (20 novos). Templates `.j2` ficam nas tasks 14–16 |
