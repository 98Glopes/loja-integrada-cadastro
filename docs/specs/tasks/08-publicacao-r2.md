...
há comando de CLI usando isso ainda (`processar` continua stub, task 11); o consumo real, por
...
- Wiring de `config/composicao.py`/`cli.py` (escolher `ArmazenamentoImagensR2` a partir de
  `Configuracao`, montar `PipelineFotos` completo) — task 11, junto do comando `processar` real.
